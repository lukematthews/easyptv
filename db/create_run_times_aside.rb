class TimeProcessor
	...
	def load_all_departures
		route_type = RouteType.find_by(route_type_name: "Vline")
		# RouteType.all.each{|route_type|
			puts "Loading run times for route type: #{route_type.route_type_name}"
			r_m = ProgressMonitor.new(route_type.routes.size)

			pool = Thread.pool(4)

			route_type.routes.each { |r|
				pool.process(r) { |route|
	...
	def load_for(ro, st, dir)
		...
		(0..7).each { |day_to_load|
			...
			new_deps = departures(st.route_type, st, today+day_to_load)
	...
	def departures(route_type, stop, time)

		departures = []
		formatted_date = time.strftime("%Y-%m-%dT00:00")
		# stops.each { |stop|
		data = run("/v3/departures/route_type/#{route_type.route_type}/stop/#{stop.stop_id}?date_utc=#{formatted_date}&")
...
def test_load
	...
	rt = RouteType.find_by(route_type_name: "Vline")

	processor = TimeProcessor.new
	processor.load_all_departures
